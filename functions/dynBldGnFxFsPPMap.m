function [sn2k,S2kInds,indsNKStore,corrn,gn] = dynBldGnFxFsPPMap(data,n,inds,PP)
...
for ni=1:n
    ...
    if ni==1
        ...
        for aa=1:size(indsNK,1)
            ...
            %gnTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1);
            %onTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1);
            
            if PP==1
                gnTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1)./NPart;
                onTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1)./NPart;
            else
                gnTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1);
                onTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1);
            end
        ...
        for aa=1:size(indsNK,1)
            ...
            if PP==1
                gnTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1)./ppF;
                onTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1)./ppF;
            else
                gnTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1);
                onTemp(cellIndsStr)=mean(prod(data(:,indsNK(aa,:)),2),1);
            end
