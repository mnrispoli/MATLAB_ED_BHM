function [sn2k,S2kInds,indsNKStore,corrn,gn] = dynBldGnPdfFx(pdf,basis,n,inds)
...
for ni=1:n
    ...
    if ni==1
        ...
        for aa=1:size(indsNK,1)
            ...
            gntemp(cellInds{:})=sum(pdf.*prod(basis(:,indsNK(aa,:)),2),1);
            ontemp(cellInds{:})=sum(pdf.*prod(basis(:,indsNK(aa,:)),2),1);
        end
        gn{ni}=gntemp;
        corrn{ni}=ontemp;
